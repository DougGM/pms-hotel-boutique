...
- Lote A / #50 implementado en rama apilada sobre WEB-14:
  `OccupancyScreen` muestra ocupación por fecha y `ManualBookingScreen` crea
  reservas manuales.
...
| Reservaciones y recepción                                               | Lote A implementado              | Revisar/mergear #50; seguir con lotes B/C/D                   |
...
métodos de WEB-14. Para #50 también se validan `typecheck`, `lint`, `build` y
respuesta HTTP 200 en dev server para `/pms/occupancy` y `/pms/bookings/new`.
