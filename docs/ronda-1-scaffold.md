...
La pantalla `front-desk/screens/CheckInScreen.tsx` ya integra este flujo: carga
habitaciones asignables, permite ejecutar `bookingService.assignRoom` y ofrece
el botón `Completar Check-in` conectado a `bookingService.checkIn`, con estados
de carga y mensajes de confirmación/error.
