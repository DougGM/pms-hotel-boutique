...
const navByRole: Record<RoleId, NavItem[]> = {
  ...
  housekeeping: [
    { label: 'Inicio', icon: Gauge }, { label: 'Habitaciones', icon: BedDouble, badge: '4' }, { label: 'Solicitudes', icon: ClipboardList, badge: '3' }, { label: 'Historial', icon: FileText },
  ],
  'room-service': [
  ...
  guest: [
    { label: 'Inicio', icon: Home }, { label: 'Mis reservas', icon: CalendarDays }, { label: 'Mi estancia', icon: BedDouble }, { label: 'Amenidades', icon: Sparkles }, { label: 'Servicios de habitación', icon: ClipboardList }, { label: 'Room service', icon: Package }, { label: 'Mis solicitudes y pedidos', icon: FileText },
  ],
};
...
export function PrivateWorkspace({ role: workspaceRole, initialNav, sessionName, sessionRoleLabel, onLogout }: PrivateWorkspaceProps) {
  ...
  const showsGreeting = ['Resumen', 'Dashboard', 'Mi jornada', 'Mi estancia', 'Inicio', 'Pedidos activos'].includes(activeNav);
  ...
  const guestRoomServiceNotifications: NotificationItem[] = rsOrders
    .filter((order) => order.room === '402' && !['Entregado', 'Cancelado', 'Rechazado'].includes(order.status))
    .slice(0, 2)
    .map((order) => ({
  ...
  const notificationItems: NotificationItem[] = activeRole === 'admin' ? [
    { title: 'Stock bajo: Toallas de baño', detail: '48 unidades disponibles, mínimo 50', tone: 'warning' },
    { title: 'Stock bajo: Shampoo', detail: '25 unidades disponibles, mínimo 40', tone: 'warning' },
    { title: 'Stock bajo: Café molido', detail: '8 unidades disponibles, mínimo 15', tone: 'warning' },
    { title: 'Stock bajo: Toallas de piscina', detail: '12 unidades disponibles, mínimo 25', tone: 'warning' },
  ] : activeRole === 'housekeeping' ? [
    { title: `${hkRooms.filter((room) => room.status === 'Pendiente').length} habitaciones pendientes`, detail: 'Revisa la cola de limpieza de hoy', tone: 'warning' },
    { title: `${hkRequests.filter((request) => request.status !== 'Completada').length} solicitudes activas`, detail: 'Amenidades y pedidos de huéspedes', tone: 'info' },
  ] : activeRole === 'reception' ? [
    { title: `${recReservationList.filter((reservation) => reservation.status === 'Pendiente').length} reservas pendientes`, detail: 'Confirma o da seguimiento desde Reservas', tone: 'warning' },
    { title: `${recRoomList.filter((room) => room.status === 'Limpieza').length} habitaciones en limpieza`, detail: 'Valida disponibilidad antes de asignar', tone: 'info' },
  ] : activeRole === 'room-service' ? [
    { title: `${rsOrders.filter((order) => order.status === 'Pendiente').length} pedidos pendientes`, detail: 'Acepta o rechaza los pedidos nuevos', tone: 'warning' },
    { title: `${rsOrders.filter((order) => order.status === 'Listo').length} pedidos listos`, detail: 'Coordina entrega a habitación', tone: 'success' },
  ] : activeRole === 'concierge' ? [
    { title: `${cgRequests.filter((request) => request.status === 'Pendiente').length} solicitudes pendientes`, detail: 'Transporte, reservas y servicios especiales', tone: 'warning' },
    { title: `${cgRequests.filter((request) => request.priority === 'Alta' && request.status !== 'Completada').length} prioridades altas`, detail: 'Atender primero durante el turno', tone: 'info' },
  ] : activeRole === 'guest' ? [
    { title: 'Reserva activa', detail: 'Suite Aurora · Habitacion 402 · Check-out 28 ago', tone: 'success' },
    ...guestRoomServiceNotifications,
    { title: 'Cargo de Room Service', detail: '$560.00 agregado a tu cuenta de estancia', tone: 'info' },
  ] : [
    { title: 'Reserva activa', detail: 'Tu estancia y servicios están disponibles', tone: 'success' },
  ];
  ...
  const addRoomServiceChargeToFolio = (order: RoomServiceOrder) => {
    ...
    setRecReservationList((current) => current.map((reservation) => {
  ...
  const updateRoomServiceOrder = (orderId: number, status: OrderStatus) => {
    ...
    setRsOrders((current) => current.map((item) => item.id === orderId ? { ...item, status, charged: status === 'Entregado' ? true : item.charged } : item));
    notify(status === 'Entregado' ? `Pedido #${orderId} entregado y cargado al folio` : `Pedido #${orderId} actualizado: ${status}`);
  };
    ...
    <div className="app-shell">
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <nav className="side-nav">
          ...
          {nav.map((item) => { const Icon = item.icon; return <button key={item.label} className={`nav-item ${activeNav === item.label ? 'active' : ''}`} onClick={() => { if (activeRole === 'reception') closeAllReceptionModals(); setActiveNav(item.label); setSidebarOpen(false); setShowNotifications(false); notify(`${item.label} seleccionado`); }}><Icon size={18} /><span>{item.label}</span>{item.badge && <b>{item.badge}</b>}</button>; })}
        </nav>
        <div className="sidebar-bottom"><div className="support-card"><div className="support-dot"><Headphones size={15} /></div><div><strong>¿Necesitas ayuda?</strong><span>Habla con soporte</span></div><ArrowRight size={15} /></div></div>
      </aside>
      {sidebarOpen && <div className="sidebar-backdrop" onClick={() => setSidebarOpen(false)} />}
      <main className="main-area">
        <header className="topbar"><button className="hamburger" onClick={() => setSidebarOpen(true)}><Menu size={22} /></button><div className="crumbs"><span>Hotel Aurora</span><span>/</span><strong>{activeNav}</strong></div><div className="topbar-actions"><div className="notification-wrap"><button className="icon-btn notification" onClick={() => { setShowNotifications((value) => !value); setShowRoleMenu(false); }} aria-label="Notificaciones"><Bell size={19} />{notificationItems.length > 0 && <i />}</button>{showNotifications && <div className="notification-menu"><div className="notification-head"><strong>Notificaciones</strong><span>{notificationItems.length}</span></div>{notificationItems.map((item) => <button key={`${item.title}-${item.detail}`} type="button" className="notification-row" onClick={() => { setShowNotifications(false); notify(item.title); }}><span className={`notification-dot ${item.tone}`} /> <span><strong>{item.title}</strong><small>{item.detail}</small></span></button>)}</div>}</div><div className="profile-wrap"><button className="profile-button" onClick={() => { setShowRoleMenu(!showRoleMenu); setShowNotifications(false); }}><span className={`avatar ${role.color}`}>{role.initials}</span><span><strong>{accountName}</strong><small>{accountRoleLabel}</small></span><ChevronDown size={15} /></button>{showRoleMenu && <div className="role-menu"><button onClick={() => { setShowRoleMenu(false); setAccountPanel('profile'); }}><UserRound size={15} /> Mi perfil</button><button onClick={() => { setShowRoleMenu(false); setAccountPanel('preferences'); }}><Settings size={15} /> Preferencias</button><button className="logout" onClick={() => { setShowRoleMenu(false); setSidebarOpen(false); onLogout?.(); }}><LogOut size={15} /> Cerrar sesión</button></div>}</div></div></header>
        <div className="content">
          <section className="welcome-row"><div><p className="eyebrow">{new Date().toLocaleDateString('es-MX', { weekday: 'long', day: 'numeric', month: 'long' })}</p><h1>{showsGreeting ? `Buenos días, ${greetingName}` : activeNav}</h1><p className="muted">{getSubtitle(activeNav, role.name)}</p></div>{activeRole !== 'guest' && <div className="welcome-actions">{activeRole === 'housekeeping' ? <button className="button primary" onClick={() => setHkShowDefectModal(true)}><Plus size={17} /> Reportar desperfecto</button> : activeRole === 'reception' ? <><button className="button secondary" onClick={() => notify('Informe preparado para descargar')}><FileText size={16} /> Exportar informe</button>{(activeNav === 'Resumen' || activeNav === 'Reservas' || activeNav === 'Calendario') && <button className="button primary" onClick={() => setRecShowNewRes(true)}><Plus size={17} /> Nueva reserva</button>}{activeNav === 'Habitaciones' && <button className="button primary" onClick={() => setRecShowBlock(true)}><Ban size={17} /> Bloquear habitación</button>}</> : <>{(activeRole !== 'admin' || activeNav === 'Dashboard') && <><button className="button secondary" onClick={() => notify('Reporte preparado para descargar')}><FileText size={16} /> Exportar reporte</button><button className="button primary" onClick={() => setShowModal(true)}><Plus size={17} /> Nueva operación</button></>}</>}</div>}</section>
      ...
      {accountPanel === 'profile' && <AccountProfileModal name={accountName} roleLabel={accountRoleLabel} initials={role.initials} tone={role.color} onClose={() => setAccountPanel(null)} onSave={notify} />}
      {accountPanel === 'preferences' && <AccountPreferencesModal name={accountName} roleLabel={accountRoleLabel} initials={role.initials} tone={role.color} initialView={activeNav} viewOptions={preferenceViews} onClose={() => setAccountPanel(null)} onSave={notify} />}
...
function HousekeepingContent({
  ...
  onOpenDetail, onOpenDefectModal, onOpenStatusModal, onSubmitDefect, onAction, onNavigate, detailRoom, onCloseDetail,
...
function StatusModal({ room, onClose, onChangeStatus }: { room: CleaningRoom; onClose: () => void; onChangeStatus: (id: number, status: RoomStatus) => void }) {
  return <div className="modal-backdrop" onMouseDown={onClose}><div className="modal" style={{ width: 360 }} onMouseDown={(e) => e.stopPropagation()}><div className="modal-head"><div><p className="eyebrow">CAMBIAR ESTADO</p><h2>Habitación {room.number}</h2></div><button className="icon-btn" onClick={onClose}><X size={18} /></button></div><p className="login-helper">Selecciona el nuevo estado de la habitación.</p><div className="hk-status-options"><button className="hk-status-option" onClick={() => onChangeStatus(room.id, 'Pendiente')}><span className="hk-summary-dot warning" /> Pendiente</button><button className="hk-status-option" onClick={() => onChangeStatus(room.id, 'En proceso')}><span className="hk-summary-dot info" /> En proceso</button><button className="hk-status-option" onClick={() => onChangeStatus(room.id, 'Completada')}><span className="hk-summary-dot success" /> Completada</button></div></div></div>;
}
