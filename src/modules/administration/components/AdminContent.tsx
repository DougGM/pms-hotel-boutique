...
type AdminReportTab = 'Ocupación' | 'Ingresos' | 'Reservas' | 'Cancelaciones' | 'Canales' | 'Servicios' | 'Temporadas';
...
const dashboardSeries: Record<DashboardPeriod, Record<'Ocupación' | 'Ingresos' | 'Reservas', { data: number[]; labels: string[]; value: string; change: string }>> = {
  Hoy: {
    Ocupación: { data: [42, 48, 55, 61, 68, 74, 79], labels: ['06:00', '09:00', '12:00', '15:00', '18:00'], value: '79.0%', change: '+3.2% vs. ayer' },
    Ingresos: { data: [18, 24, 31, 44, 52, 61, 74], labels: ['06:00', '09:00', '12:00', '15:00', '18:00'], value: '$7.4K', change: '+5.1% vs. ayer' },
    Reservas: { data: [2, 4, 5, 8, 9, 12, 14], labels: ['06:00', '09:00', '12:00', '15:00', '18:00'], value: '14', change: '+2 hoy' },
  },
  '7 días': {
    Ocupación: { data: [71, 76, 74, 80, 83, 78, 86], labels: ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'], value: '78.3%', change: '+4.6% vs. semana anterior' },
    Ingresos: { data: [82, 94, 88, 105, 122, 118, 130], labels: ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'], value: '$73.9K', change: '+9.8% vs. semana anterior' },
    Reservas: { data: [18, 22, 19, 26, 31, 28, 34], labels: ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'], value: '178', change: '+12 reservas' },
  },
  '30 días': {
    Ocupación: { data: [62, 68, 65, 72, 70, 78, 75, 82, 79, 85, 88, 84, 90, 87, 92], labels: ['01 ago', '05 ago', '10 ago', '15 ago', '20 ago', '25 ago', '30 ago'], value: '84.6%', change: '+8.2% vs. mes anterior' },
    Ingresos: { data: [98, 112, 105, 128, 135, 142, 128, 152, 148, 166, 172, 164, 181, 176, 190], labels: ['01 ago', '05 ago', '10 ago', '15 ago', '20 ago', '25 ago', '30 ago'], value: '$128.4K', change: '+14.5% vs. mes anterior' },
    Reservas: { data: [28, 35, 32, 40, 38, 45, 42, 48, 46, 52, 55, 50, 58, 56, 61], labels: ['01 ago', '05 ago', '10 ago', '15 ago', '20 ago', '25 ago', '30 ago'], value: '642', change: '+8.9% vs. mes anterior' },
  },
  '90 días': {
    Ocupación: { data: [58, 64, 69, 73, 76, 81, 84, 86, 89], labels: ['Jun', 'Jul', 'Ago', 'Sep'], value: '81.2%', change: '+10.4% vs. trimestre anterior' },
    Ingresos: { data: [260, 282, 304, 336, 352, 371, 390, 415, 438], labels: ['Jun', 'Jul', 'Ago', 'Sep'], value: '$438K', change: '+18.1% vs. trimestre anterior' },
    Reservas: { data: [118, 130, 142, 158, 171, 184, 192, 204, 218], labels: ['Jun', 'Jul', 'Ago', 'Sep'], value: '1,517', change: '+15.3% vs. trimestre anterior' },
  },
};
const reportPeriodData: Record<ReportPeriod, { labels: string[]; occupancy: number[]; income: number[]; reservations: number[]; cancellations: number[]; occupancyValue: string; incomeValue: string; reservationsValue: string; cancellationValue: string; trend: string; change: string }> = {
  Día: { labels: ['06', '09', '12', '15', '18', '21'], occupancy: [42, 51, 58, 65, 72, 79], income: [18, 24, 31, 44, 52, 61], reservations: [2, 4, 5, 8, 9, 12], cancellations: [1, 0, 1, 2, 1, 0], occupancyValue: '79.0%', incomeValue: '$7.4K', reservationsValue: '14', cancellationValue: '5', trend: 'En aumento', change: '+3.2%' },
  Semana: { labels: ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'], occupancy: [71, 76, 74, 80, 83, 78, 86], income: [82, 94, 88, 105, 122, 118, 130], reservations: [18, 22, 19, 26, 31, 28, 34], cancellations: [3, 4, 2, 5, 3, 2, 4], occupancyValue: '78.3%', incomeValue: '$73.9K', reservationsValue: '178', cancellationValue: '23', trend: 'Estable', change: '+4.6%' },
  Mes: { labels: ['S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7'], occupancy: [62, 68, 65, 72, 70, 78, 75, 82, 79, 85, 88, 84, 90, 87, 92], income: [98, 112, 105, 128, 135, 142, 128], reservations: [38, 45, 32, 27, 41, 48, 52], cancellations: [15, 12, 10, 14, 8, 11, 9], occupancyValue: '84.6%', incomeValue: '$128.4K', reservationsValue: '241', cancellationValue: '12', trend: 'Ascendente', change: '+8.2%' },
  Año: { labels: ['Ene', 'Mar', 'May', 'Jul', 'Sep', 'Nov'], occupancy: [65, 70, 74, 78, 82, 88], income: [240, 268, 292, 315, 344, 372], reservations: [420, 452, 488, 510, 548, 590], cancellations: [38, 34, 31, 28, 26, 24], occupancyValue: '81.8%', incomeValue: '$1.92M', reservationsValue: '5,840', cancellationValue: '8.1%', trend: 'Crecimiento', change: '+11.6%' },
  Temporada: { labels: ['Baja', 'Media', 'Alta', 'Festiva'], occupancy: [64, 78, 91, 96], income: [228, 321, 482, 536], reservations: [280, 420, 580, 640], cancellations: [11, 8, 6, 5], occupancyValue: '88.4%', incomeValue: '$536K', reservationsValue: '640', cancellationValue: '5.0%', trend: 'Alta demanda', change: '+16.9%' },
};
...
function StatusSwitch({ checked, label, onChange }: { checked: boolean; label: string; onChange: () => void }) {
  return (
    <button
      type="button"
...
function EditIconButton({ label, onClick }: { label: string; onClick: () => void }) {
  return (
    <button className="button small secondary adm-icon-action" type="button" aria-label={label} title={label} onClick={onClick}>
      <Pencil size={14} />
    </button>
  );
}
...
function MiniChart({ data, labels, color = '#b88d50' }: { data: number[]; labels: string[]; color?: string }) {
  ...
  const chartLabels = labels.length <= 5
    ? labels
    : labels.filter((_, index) => index === 0 || index === Math.floor((labels.length - 1) / 2) || index === labels.length - 1);
  return (
    <div className="chart-area">
      <div className="chart">
        ...
        <div className="chart-x">{chartLabels.map((l) => <span key={l}>{l}</span>)}</div>
      </div>
    </div>
  );
}
...
function AdminContentReady({
  ...
  const [dashboardTab, setDashboardTab] = useState<'Ocupación' | 'Ingresos' | 'Reservas'>('Ocupación');
  ...
  if (nav === 'Dashboard') {
    ...
    const dashboardLegend = dashboardTab === 'Ocupación' ? 'Ocupación promedio' : dashboardTab === 'Ingresos' ? 'Ingresos acumulados' : 'Reservas confirmadas';
    return (
      <>
        <div className="dashboard-grid">
          <div className="panel main-panel">
            <div className="panel-heading">
              ...
              <button className="text-button" onClick={() => { if (onNavigate) onNavigate('Reportes'); else onAction('Reportes seleccionado'); }}>Ver todo <ArrowRight size={15} /></button>
            </div>
            <div className="chart-tabs">
              {(['Ocupación', 'Ingresos', 'Reservas'] as const).map((tab) => (
                <button key={tab} className={dashboardTab === tab ? 'active' : ''} onClick={() => setDashboardTab(tab)}>{tab}</button>
              ))}
              <div className="filter-dropdown">
                <button className="filter-button"><span>{dashboardPeriod}</span><ChevronDown size={14} /></button>
                <div className="filter-menu">
                  {dashboardPeriodOptions.map((period) => (
                    <button key={period} className={dashboardPeriod === period ? 'active' : ''} onClick={() => setDashboardPeriod(period)}>{period}</button>
                  ))}
                </div>
              </div>
            </div>
            <MiniChart data={dashboardChart.data} labels={dashboardChart.labels} color={dashboardTab === 'Reservas' ? '#5f7f9d' : undefined} />
            <div className="chart-footer">
              <span><i className="legend-dot" /> {dashboardLegend}</span>
        ...
        <div className="bottom-grid">
          <div className="panel occupancy-panel">
            <div className="panel-heading">
              ...
              <button className="button small secondary" onClick={() => { if (onNavigate) onNavigate('Habitaciones'); else onAction('Habitaciones seleccionado'); }}>Gestionar <ArrowRight size={14} /></button>
            </div>
            ...
            {filtered.map((u) => (
              <tr key={u.id}>
                ...
                <td className="adm-actions">
                  <EditIconButton label="Editar usuario" onClick={() => { setEditUser(u); setShowUserModal(true); }} />
                  <StatusSwitch checked={u.status === 'Activo'} label={u.status === 'Activo' ? 'Desactivar usuario' : 'Activar usuario'} onChange={() => { setUsers((cur) => cur.map((x) => x.id === u.id ? { ...x, status: x.status === 'Activo' ? 'Inactivo' : 'Activo' } : x)); onAction(`Usuario ${u.name} ${u.status === 'Activo' ? 'desactivado' : 'activado'}`); }} />
                </td>
              </tr>
            ))}
          </AdminTable>
            ...
            {roles.map((r) => (
              <div className="adm-role-card" key={r.id}>
                ...
                <div className="adm-role-actions">
                  <EditIconButton label="Editar rol" onClick={() => { setEditRole(r); setShowRoleModal(true); }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <AdminTable headers={['Número', 'Piso', 'Tipo', 'Capacidad', 'Tarifa', 'Estado', 'Acciones']}>
            {filtered.map((r) => (
              <tr key={r.id}>
                ...
                <td className="adm-actions">
                  <EditIconButton label="Editar habitación" onClick={() => { setEditRoom(r); setShowRoomModal(true); }} />
                  <StatusSwitch checked={r.status !== 'Mantenimiento'} label={r.status === 'Mantenimiento' ? 'Activar habitación' : 'Desactivar habitación'} onChange={() => { setRooms((cur) => cur.map((x) => x.id === r.id ? { ...x, status: x.status === 'Mantenimiento' ? 'Disponible' : 'Mantenimiento' } : x)); onAction(`Habitación ${r.number} ${r.status === 'Mantenimiento' ? 'activada' : 'desactivada'}`); }} />
                </td>
              </tr>
            ))}
          </AdminTable>
          ...
          <div className="adm-roomtype-grid">
            {roomTypes.map((rt) => (
              <div className="adm-roomtype-card" key={rt.id}>
                ...
                <div className="adm-role-actions">
                  <EditIconButton label="Editar tipo de habitación" onClick={() => { setEditRoomType(rt); setShowRoomTypeModal(true); }} />
                  <StatusSwitch checked={rt.status === 'Activo'} label={rt.status === 'Activo' ? 'Desactivar tipo' : 'Activar tipo'} onChange={() => { setRoomTypes((cur) => cur.map((x) => x.id === rt.id ? { ...x, status: x.status === 'Activo' ? 'Inactivo' : 'Activo' } : x)); onAction(`Tipo ${rt.name} ${rt.status === 'Activo' ? 'desactivado' : 'activado'}`); }} />
                </div>
              </div>
            ))}
          </div>
          ...
          <AdminTable headers={['Tipo de habitación', 'Temporada', 'Fechas', 'Tarifa base', 'Tarifa especial', 'Estado', 'Acciones']}>
            {seasonRates.map((sr) => (
              <tr key={sr.id}>
                ...
                <td className="adm-actions">
                  <StatusSwitch checked={sr.status === 'Activa'} label={sr.status === 'Activa' ? 'Desactivar tarifa' : 'Activar tarifa'} onChange={() => { setSeasonRates((cur) => cur.map((x) => x.id === sr.id ? { ...x, status: x.status === 'Activa' ? 'Inactiva' : 'Activa' } : x)); onAction(`Tarifa ${sr.status === 'Activa' ? 'desactivada' : 'activada'}`); }} />
                </td>
              </tr>
            ))}
          </AdminTable>
          ...
          <div className="adm-dynrate-grid">
            {dynamicRates.map((dr) => (
              <div className="adm-dynrate-card" key={dr.id}>
                ...
                <div className="adm-role-actions">
                  <StatusSwitch checked={dr.status === 'Activa'} label={dr.status === 'Activa' ? 'Desactivar regla' : 'Activar regla'} onChange={() => { setDynamicRates((cur) => cur.map((x) => x.id === dr.id ? { ...x, status: x.status === 'Activa' ? 'Inactiva' : 'Activa' } : x)); onAction(`Regla ${dr.status === 'Activa' ? 'desactivada' : 'activada'}`); }} />
                </div>
              </div>
            ))}
          </div>
              ...
              return (
                <div className="adm-amenity-card" key={a.id}>
                  ...
                  <div className="adm-amenity-actions">
                    <EditIconButton label="Editar amenidad" onClick={() => { setEditAmenity(a); setShowAmenityModal(true); }} />
                    <StatusSwitch checked={a.status === 'Activo'} label={a.status === 'Activo' ? 'Desactivar amenidad' : 'Activar amenidad'} onChange={() => { setAmenities((cur) => cur.map((x) => x.id === a.id ? { ...x, status: x.status === 'Activo' ? 'Inactivo' : 'Activo' } : x)); onAction(`Amenidad ${a.status === 'Activo' ? 'desactivada' : 'activada'}`); }} />
                  </div>
                </div>
              );
            })}
          ...
          <div className="adm-rs-grid">
            {filteredRs.map((item) => (
              <div className="adm-rs-card" key={item.id}>
                ...
                <div className="adm-amenity-actions">
                  <EditIconButton label="Editar producto de room service" onClick={() => { setEditRsItem(item); setShowRsItemModal(true); }} />
                  <StatusSwitch checked={item.status === 'Activo'} label={item.status === 'Activo' ? 'Desactivar producto' : 'Activar producto'} onChange={() => { setRsItems((cur) => cur.map((x) => x.id === item.id ? { ...x, status: x.status === 'Activo' ? 'Inactivo' : 'Activo' } : x)); onAction(`Producto ${item.status === 'Activo' ? 'desactivado' : 'activado'}`); }} />
                </div>
              </div>
            ))}
          </div>
    ...
    return (
      <div className="panel">
        ...
        <div className="adm-report-filters">
          {reportPeriodOptions.map((f) => <button key={f} className={`adm-filter-chip ${reportFilter === f ? 'active' : ''}`} onClick={() => setReportFilter(f)}>{f}</button>)}
        </div>
        <div className="adm-report-content">
          {reportTab === 'Ocupación' && (
            <>
              <div className="adm-report-stats">
                <div className="metric-card"><div className="metric-icon sage"><BedDouble size={19} /></div><div><p>Ocupación promedio</p><h2>{reportData.occupancyValue}</h2><span className="positive">{reportData.change}</span></div></div>
                <div className="metric-card"><div className="metric-icon gold"><TrendingUp size={19} /></div><div><p>Tendencia</p><h2>{reportData.trend}</h2><span className="positive">{reportFilter}</span></div></div>
              </div>
          ...
          {reportTab === 'Ingresos' && (
            <>
              <div className="adm-report-stats">
                <div className="metric-card"><div className="metric-icon gold"><Wallet size={19} /></div><div><p>Ingresos del periodo</p><h2>{reportData.incomeValue}</h2><span className="positive">{reportData.change}</span></div></div>
                <div className="metric-card"><div className="metric-icon sage"><DollarSign size={19} /></div><div><p>Promedio por corte</p><h2>${Math.round(reportData.income.reduce((sum, value) => sum + value, 0) / reportData.income.length).toLocaleString()}K</h2><span className="positive">{reportFilter}</span></div></div>
              </div>
          ...
          {reportTab === 'Reservas' && (
            <AdminTable headers={['Periodo', 'Reservas', 'Check-ins', 'Check-outs', 'Ingresos']}>
              {reservationRows.map((row) => (
                <tr key={row.label}><td>{row.label}</td><td>{row.reservations}</td><td>{row.checkIns}</td><td>{row.checkOuts}</td><td>${row.income.toLocaleString()}</td></tr>
              ))}
            </AdminTable>
          )}
          {reportTab === 'Cancelaciones' && (
            <>
              <div className="adm-report-stats">
                <div className="metric-card"><div className="metric-icon terracotta"><Ban size={19} /></div><div><p>Cancelaciones</p><h2>{reportData.cancellationValue}</h2><span>-2.4%</span></div></div>
                <div className="metric-card"><div className="metric-icon info"><Percent size={19} /></div><div><p>Tasa de cancelación</p><h2>{reportFilter === 'Día' ? '3.1%' : reportFilter === 'Temporada' ? '5.0%' : '8.5%'}</h2><span className="positive">-1.2%</span></div></div>
              </div>
              <MiniChart data={reportData.cancellations} labels={reportData.labels} color="#a9483c" />
            </>
          )}
          {reportTab === 'Canales' && (
