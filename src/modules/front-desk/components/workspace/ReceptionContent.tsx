...
import type { Companion, FolioEntry, PaymentMethod, RecRoom, Reservation, ReservationStatus, RoomBlock } from '@/private/workspace/PrivateWorkspace';
...
export function ReceptionContent({
  ...
  calFilter, setCalFilter, search, setSearch, onAction, onNavigate,
}: {
  nav: string;
        ...
        <section className="dashboard-grid">
          ...
          <div className="panel side-panel">
            ...
            <div className="rc-quick-actions">
              ...
              <button onClick={() => onNavigate('Caja')}><Wallet size={16} /> Caja</button>
            </div>
          </div>
        </section>
  ...
  if (nav === 'Huéspedes') {
    const results = search.trim()
      ? reservations.filter((r) => `${r.guest.name} ${r.guest.lastName} ${r.guest.phone} ${r.guest.email} ${r.code}`.toLowerCase().includes(search.toLowerCase()))
    ...
    return (
      <div className="panel">
        ...
        <div className="rc-avail-form">
          <label className="rc-field"><span>Entrada</span><input type="date" value={availCheckIn} onChange={(e) => setAvailCheckIn(e.target.value)} /></label>
          <label className="rc-field"><span>Salida</span><input type="date" value={availCheckOut} onChange={(e) => setAvailCheckOut(e.target.value)} /></label>
          <label className="rc-field"><span>Huéspedes</span><input type="number" min={1} value={availGuests} onChange={(e) => setAvailGuests(Number(e.target.value) || 1)} /></label>
          <label className="rc-field"><span>Tipo</span><select value={availType} onChange={(e) => setAvailType(e.target.value)}><option>Todos</option><option>Estándar</option><option>Deluxe</option><option>Suite</option></select></label>
          <button className="button primary" onClick={() => onAction(`Disponibilidad consultada del ${availCheckIn} al ${availCheckOut}`)}><Search size={16} /> Buscar</button>
        </div>
